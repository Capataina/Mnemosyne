...
just lynceus-mas-package                            # real MAS .pkg — real identities (filled 2026-08-04),
                                                    # embeds the profile, strips xattrs, then productbuild
...
Lynceus's version lives in three text manifests that move together in the same commit, with `Cargo.lock` following: `apps/lynceus/package.json`, `apps/lynceus/src-tauri/Cargo.toml`, `apps/lynceus/src-tauri/tauri.conf.json` — all `1.0.0` today (the 1.0 line was crossed 2026-08-04 with the founder's explicit yes, for the first App Store submission). The engine (`crates/engine/Cargo.toml`, `0.5.6`) versions independently and bumps only when its own code changes. The root `package.json` (`0.2.0`) tracks monorepo-level structure, not either product.
...
Lynceus 1.0.0 (the 1.0 gate crossed on submission day, 2026-08-04), engine 0.5.6. Submission day is in progress: certificates + App ID + provisioning profile exist (the profile is committed beside the entitlements), `just lynceus-mas-package` produces a verified signed .pkg (WWDR G3 intermediate trap solved — see the packaging commit 1d783d6; the recipe also strips extended attributes before signing after Apple's 91109 quarantine bounce, 4b88b9d), the live folder-persistence test PASSED founder-driven on the sandbox build, all five founder decisions are closed (£9.99 UK base, Silicon-only, legal-name seller, copy + screenshots signed off), and the ASC record exists with listing fields going in. The 1.0.0 .pkg has been delivered via Transporter, processed by Apple, and attached to the ASC version record with the encryption-compliance question answered. Remaining: the Paid Apps Agreement's tax form (blocked ~24h on bank verification), price entry, and the submit button. The EU DSA trader declaration is deliberately deferred (home-address publication question). Tests green: engine 143/143, frontend 294/294. The knowledge layer settled today: the stranded 24-July Hermes migration was recovered (12d1712, 31217fc), every folder gained a CLAUDE.md, and the `docs/` tree was dissolved into those files — per-folder CLAUDE.md is now the only documentation layer.
