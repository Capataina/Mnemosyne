...
Submitted-shaped: at 1.0.0, signed with the real Apple Distribution + Mac Installer Distribution identities, packaged by `just lynceus-mas-package`, delivered to App Store Connect through Transporter and attached to its version record. Every repo-side gate is closed — enrolment (2026-08-03), certificates + App ID + Mac App Store provisioning profile (2026-08-04), the live folder-persistence test (passed founder-driven, 2026-08-04), and all five founder decisions (£9.99 UK base storefront, Apple Silicon only, legal-name seller, listing copy and the eight screenshot pages signed off). What is left lives in App Store Connect, not here: the Paid Applications Agreement's tax form, price entry, and the submit button. Sandbox entitlements are wired (read-only user-selected folders + security-scoped bookmarks + `network.client`, which WKWebView requires to render at all under the sandbox — added 2026-08-03 after two blank-window live tests), the 674MB bundle ships five int8 models plus two tokenizers, and the sealed app's own code makes zero network requests (boot-log verified, 5968d2e). Repo-side gates cleared: real icon (b5005e4), bundle ID `com.capataina.lynceus` (f14aaa8), listing copy drafted then de-em-dashed and soft-wrapped for direct paste into App Store Connect (ffab7e3), the store-signing path live (1d783d6, 4b88b9d), and `design/store/release-runbook.md`'s 5-minute live folder-persistence test passed (b77145a).

## Gaps and planned work

- **For 1.0.1 (founder-queued 2026-08-04, deliberately not done mid-submission):** set `ITSAppUsesNonExemptEncryption = false` in the bundled Info.plist (Tauri injects plist keys via `tauri.conf.json`) so App Store Connect stops asking the export-compliance encryption question on every build upload. The 1.0.0 answer was given by hand in the ASC dialog ("none of the algorithms").
