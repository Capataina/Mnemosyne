...
- `model_download.rs:12` says "All FP32 (no quantization)" — true of what it downloads, misleading against the int8-default store reality. [code-health-audit 2026-08-02]
- `indexing.rs:1129-1134` claims each encoder thread opens its own span via the encoder runners' instrumentation — neither runner carries any tracing attribute. [code-health-audit 2026-08-02]
- **The feed-delta emit-ordering contract is untested on the emitting side** (coverage gap): the flush-before-terminal-phase-emit invariant and 64-row batching (~lines 765-795) have no backend test; frontend reconciliation is tested, the emitter is prose-only. A reorder would drop the tail delta batch — pop-in loss invisible to both suites. [code-health-audit 2026-08-02]

## Planned work

Ordered by severity. All entries behaviour-free with proofs referenced; from the 2026-08-02 audit.

- **MED-HIGH — split `indexing.rs` (1828 lines) into an `indexing/` module folder** (modularisation). Five seams, address-only, with the amended-complete batch (a refuter killed the original "pure address move" claim and reissued): external references at `lib.rs:17,34,501`, `watcher.rs:26,56,82,117,125`, `commands/roots.rs:8,102,154`, plus three CLAUDE.md lines and the doc mention at `commands/images.rs:21` (`run_pipeline_inner`); cross-seam privates `run_encoder_phase` and `emit_preprocessing_sample` widen to `pub(super)`; `emit`, `emit_feed_delta`, `FEED_DELTA_BATCH`, and `FusionSlots` (line 58) must live in `mod.rs` itself or each needs `pub(super)`; the `#[cfg(test)]` module (1656-1828) splits — the three `encode_progress_*` tests read private fields and move into the encode submodule, the other seven stay in `mod.rs`. Wire contracts untouched by construction. [code-health-audit 2026-08-02]
- **MED — Ready count via `COUNT(*)`** — edit site `indexing.rs:1008`; the full entry (numbers, proofs, connection note) lives in `crates/engine/src/db/CLAUDE.md`'s planned work. [code-health-audit 2026-08-02]
- **MED — delete `run_clip_encoder_with_intra` (~100 lines) in favour of `run_trait_encoder`** (pattern extraction). Gate token-diffed lines 1351-1482 vs 1486-1588: every divergence is parameterised by `run_trait_encoder(app, db, "clip_vit_b_32", || ClipImageEncoder::new_with_intra(&path, intra), &progress)`; the trait impl provably forwards `encode_batch` to the inherent method (`encoder.rs:368-373`), CLIP's batch-1 constraint lives inside it, and neither function carries a span to diverge. [code-health-audit 2026-08-02]
- **MED — extract the triplicated bounded-pool + monotonic-emit block, and the `/4000` cadence constant with it** (pattern extraction). Copies at 598-630, 711-790, 917-973 (emit) and 579/633-643 vs 909/980-990 (pool — one `prefix` parameter covers the divergence); `EncodeProgress` is the already-extracted fourth instance and its unit tests pin the semantics. Two carve-outs stay put at the thumbnail site: the `delta_buffer` logic (733-796) and the unconditional terminal emit (798, fires even at total==0). Note: adopting `EncodeProgress` strengthens `Relaxed` → `SeqCst` on the counter — emitted value set unchanged (decisions happen under the mutex), test-pinned. [code-health-audit 2026-08-02]
- **LOW — the encode-progress denominator via COUNT** (`indexing.rs:1198`): equivalence gate-proven, but the headline 2.5× was debug-profile; release-serial is 1.25% — the honest win is allocation churn only. Proof: `crates/engine/tests/cha_b_needs_count.rs`. [code-health-audit 2026-08-02]
- **LOW — move the state types + `spawn_cache_warm` out of `lib.rs` behind crate-root `pub use`** (system extraction). Every importer already uses `use crate::{…}` paths (verified: similarity.rs:11, semantic.rs:13, semantic_fused.rs:56, images.rs:13, roots.rs:13, indexing.rs:52), so the move is invisible to call sites; the profiling-only setup blocks (lib.rs 392-463) extract file-local. [code-health-audit 2026-08-02]
- **LOW — publish shared aliases for the managed-state types** (inconsistent patterns). `FusionSlots` declarations at indexing.rs:58 ≡ watcher.rs:57,126 ≡ lib.rs:63,254 and `WatcherSlot` at roots.rs:19 ≡ lib.rs:348 are proven textually identical (the values already flow between the sites, so the compiler has unified them); one published alias pair turns the exact-type managed-state trap into a compile-time invariant. [code-health-audit 2026-08-02]
- **LOW — the eleven-item doc-rot batch**: the five stale-doc notes above plus `profiling.rs:4` and `main.rs`'s `--profile` comments, `main.rs`'s dead `let _ = profiling;`, `encoders.rs`'s retired `embedding_clip` column + false `Box<dyn ImageEncoder>` claims, `encoder.rs:316-320`'s false "tests still call it" (`encode_all_images_in_database` has zero callers — itself grep-proven dead code deletable in the same pass, along with `generate_all_missing_thumbnails` and the two `inspect_model`s). No doc-tests in the crate (0 collected), so all comment edits are inert. [code-health-audit 2026-08-02]
