...
├── lib.rs            run(): manages state, setup callback (bundled-resource models
│                     dir, startup profiling diagnostics, legacy scan_root migration,
│                     security scopes, pipeline + watcher spawn), invoke_handler
│                     registry, exit-time perf report
├── state.rs          managed-state types (FusionIndexState, TextEncoderState),
│                     spawn_cache_warm, and the shared FusionSlots/WatcherSlot aliases
│                     every managed-state declaration site uses — re-exported at the
│                     crate root so `crate::FusionIndexState` etc. resolve unchanged
├── commands/         the Tauri command boundary (own CLAUDE.md)
├── indexing/         background pipeline module: mod.rs owns orchestration
│                     (scan → relink → thumbnail → previews → Ready) plus the shared
│                     emit/cadence/bounded-pool helpers; encode.rs owns the per-encoder
│                     phase (EncodeProgress, run_encoder_phase, run_trait_encoder) —
│                     address-only split of the former 1746-line indexing.rs
│                     [code-health-audit 2026-08-02]. Single-flight AtomicBool; emits
│                     indexing-progress
...
## The pipeline (`indexing/`) — phase order and why it's load-bearing
...
1. **model-download** — `download_models_if_missing` with a byte-progress callback (the only `Phase::ModelDownload` event source; silent skip when files exist). Failure is `warn!` + continue: thumbnails still run, search stays unavailable until models arrive (typed `TextModelMissing` from the lazy path prompts re-download). Then pre-warm **both** text encoders (CLIP + SigLIP-2 slots in `TextEncoderState`) if their files exist — not a phase event; saves 1-2s (CLIP) / ~2.4s (SigLIP-2) off the first search. Lazy init in `commands::semantic_fused` remains the fallback when pre-warm raced the download (moved here 2026-08-03 when the legacy single-encoder `commands::semantic` module was deleted).
...
Launch-time cache warm lives in `state::spawn_cache_warm`, not the pipeline: it mmaps each enabled encoder's persisted flat store into the `FusionIndexState` slots on its own thread at `run()`, DB-populating and writing the store back on a miss. The pipeline's only cache duty is step 5.
...
- **Tauri resolves managed state by exact type** (2c07add). `FusionSlots`/`WatcherSlot` are now single published aliases in `state.rs` (2026-08-03, [code-health-audit 2026-08-02]) — every declaration site uses the same alias by construction, so the once-manual "must match lib.rs's flavour" discipline is now a compile-time invariant rather than a convention to remember.
...
- **The feed-delta emit-ordering contract is untested on the emitting side** (coverage gap): the flush-before-terminal-phase-emit invariant and 64-row batching (line numbers shifted by the 2026-08-03 content pass — search for `delta_buffer`) have no backend test; frontend reconciliation is tested, the emitter is prose-only. A reorder would drop the tail delta batch — pop-in loss invisible to both suites. [code-health-audit 2026-08-02]

Fixed 2026-08-03 (content-changes pass): `indexing.rs` header's trigger-2 single-folder narrative now describes the multi-root table; the `run_encoder_phase` doc comment's false "each encoder thread opens its own span" claim is corrected; the "thumbnails + encoders in parallel" reversal (the module doc previously implied `run_encoder_phase` ran concurrently with the thumbnail rayon loop — Phase 12b already made that strictly sequential) is corrected at its doc-comment site.

Fixed 2026-08-03 (command-surface pass): `settings.rs` header no longer says "just the scan root / Pass 4 will populate" — rewritten to name the precision/encoders/scan-root fields the struct actually carries; `watcher.rs` header's "lib.rs Phase 7" citation now points at the scan phase of `indexing.rs`; `model_download.rs:12`'s "All FP32 (no quantization)" is expanded to name this module as the fallback path only, distinct from the int8-default store build.

## Planned work

Ordered by severity. All entries behaviour-free with proofs referenced; from the 2026-08-02 audit.

- **Doc-rot batch: complete, eleven of eleven.** Landed 2026-08-03 in the command-surface pass: `profiling.rs:4` and `main.rs`'s `--profile` comments, `main.rs`'s dead `let _ = profiling;`, `encoders.rs`'s retired `embedding_clip` column + false `Box<dyn ImageEncoder>` claims, `encoder.rs:316-320`'s false "tests still call it" comment removed along with the dead code it described (`encode_all_images_in_database` deleted — zero callers, grep-confirmed; its `inspect_model` sibling in the same file deleted the same way). `generate_all_missing_thumbnails` deleted the same day (zero external callers) as part of the thumbnail needs-set entry. The last straggler — `encoder_text/encoder.rs`'s `inspect_model`, which sat between two seats' file lists — was re-verified zero-callers repo-wide and deleted in the Phase C sweep, 2026-08-03. No doc-tests in the crate (0 collected), so all comment edits were inert. [code-health-audit 2026-08-02, landed]

**Done 2026-08-03** (content-changes pass, proofs in `crates/engine/tests/`): Ready count via `COUNT(*)` (was `indexing.rs:1008`, now routes through `get_pipeline_stats().total_images`); `run_clip_encoder_with_intra` deleted in favour of `run_trait_encoder`; the triplicated bounded-pool + monotonic-emit blocks (backfill, base thumbnails, eager previews) now share `EncodeProgress` + `run_on_bounded_pool` + `bounded_pool_size` + the named `EMIT_CADENCE_DIVISOR`/`emit_cadence`; the encode-progress denominator now uses `count_images_without_embedding_for` (a new COUNT-only method, since the entry's target `indexing.rs:1198` needed a count-form sibling of `embeddings.rs::get_images_without_embedding_for`, which lives outside this pass's owned files). See `crates/engine/src/db/CLAUDE.md`'s Done note for the `images_query.rs`-side half of this batch.

**Done 2026-08-03** (CHA Phase B, the three structural moves, proofs: `cargo test --workspace` 229/0, `pnpm --filter ./apps/lynceus run test` 247/247):

- `indexing.rs` (1746 lines) became `indexing/` — `mod.rs` keeps orchestration (`try_spawn_pipeline`, `run_pipeline_inner`), the event/payload types, `emit`/`emit_feed_delta`/`FEED_DELTA_BATCH`/`FusionSlots` (kept ancestor-private per the entry's own "or stays in mod.rs" option), and — re-derived from the 2026-08-03 content pass, which the original entry predates — `EncodeProgress`, `EMIT_CADENCE_DIVISOR`, `emit_cadence`, `bounded_pool_size`, `run_on_bounded_pool` moved into `encode.rs` instead of staying in `mod.rs`, `pub(super)`, because `mod.rs`'s backfill/thumbnail/preview passes now share `EncodeProgress` with `encode.rs`'s `run_encoder_phase` (that sharing postdates the original split entry). `encode.rs` holds `run_encoder_phase` (`pub(super)`, called from `mod.rs`) and `run_trait_encoder`/`emit_preprocessing_sample` (private — re-verified against the current call graph, `emit_preprocessing_sample` is only ever called from `run_trait_encoder` in the same file, so the original entry's widening for it wasn't needed here). The 3 `encode_progress_*` tests moved into `encode.rs`'s own test module (private-field access), the other 7 stayed in `mod.rs`. One reference fix the move required and the entry didn't name: `run_encoder_phase`'s `super::similarity_and_semantic_search::…` became `crate::similarity_and_semantic_search::…` (`super` now means `indexing`, not the crate root).
- `images_query.rs` → `db/feed_manifest.rs`: exactly per the entry — `FeedManifestRow`, `basename_of` (→ `pub(super)`, since `images_query.rs`'s `get_images_without_thumbnails` still calls it), and the `get_feed_manifest`/`get_image_names_for_search`/`get_image_details_by_ids` impl block, plus the four T3-1 tests. `aggregate_image_rows` widened to `pub(super)` in `images_query.rs`. `commands/images.rs`'s import split (`FeedManifestRow` from the new path, `PipelineStats` stays). Phase A's `ThumbnailNeedRow`/`count_images_without_embedding_for` stayed in `images_query.rs` (needs-set/stats concerns, not feed-manifest).
- The managed-state types + `spawn_cache_warm` moved from `lib.rs` into a new `state.rs`, re-exported at the crate root (`pub use state::{FusionIndexState, FusionSlots, TextEncoderState, WatcherSlot}`) — every existing `crate::FusionIndexState`/`crate::TextEncoderState` call site kept resolving unchanged, as the entry predicted. `FusionSlots`/`WatcherSlot` are now the one published alias pair, used at every prior duplicate-declaration site (`indexing/mod.rs`, `watcher.rs`, `commands/roots.rs`, `lib.rs`). The profiling-only setup diagnostics (startup-state snapshot + cosine-math sanity check) extracted into a private `emit_startup_profiling_diagnostics` fn in `lib.rs`.
