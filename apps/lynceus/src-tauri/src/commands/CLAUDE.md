...
- **Attribute-less commands run on the main thread** (Tauri v2; 244b87a). The slow commands are already `#[tauri::command(async)]`: the four root mutations, `purge_orphaned_images`, `get_preview_breakdown`, and — since 2026-08-03 — `get_thumbnail` (its cold-bucket path decodes+resizes+re-encodes; a column-count change fanning every mounted tile into cold large-bucket requests froze the app on the main thread; the frontend also gained a 4-slot admission gate in `useAdaptiveThumbnail`). A new command that deletes, walks directories, or rebuilds a watcher gets the attribute; the similarity commands stay deliberately sync (fast-path — in-memory scoring only).
...
- **The `ranked_for_encoder` delegation warns on an empty encoder slot** (2026-08-03, verifier-noted, accepted): the composed path calls `get_similar_images_sorted` on the empty index, emitting a `cosine.get_similar_sorted` span + "No similarities calculated!" warn where the old inline arm was silent — reachable on `get_fused_semantic_search`'s empty-slot path only, results identical; extra trace entries under `--profiling`, not product behaviour.
