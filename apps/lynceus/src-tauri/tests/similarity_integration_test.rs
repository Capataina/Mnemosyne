...
fn test_real_image_similarity_search() {
    ...
    let query_embedding = embeddings_map
        .get(query_path)
    ...
    let self_similarity = CosineIndex::cosine_similarity(
        &Array1::from_vec(self_embedding.clone()),
        &Array1::from_vec(self_embedding.clone()),
    );

    println!(
        "Self-similarity (image compared to itself): {:.6}",
        self_similarity
    );
    assert!(
    ...
    if image_paths.len() > 1 {
        ...
        let cross_similarity = CosineIndex::cosine_similarity(
            &Array1::from_vec(embedding_a.clone()),
            &Array1::from_vec(embedding_b.clone()),
        );

        println!(
            "Cross-similarity (first vs second image): {:.6}",
            cross_similarity
        );
...
fn test_similarity_distribution() {
    ...
    let sample_paths: Vec<&std::path::Path> = image_paths
        .iter()
        .take(sample_size)
        .map(|p| p.as_path())
        .collect();

    let embeddings_vec = encoder
        .encode_batch(&sample_paths)
        .expect("Failed to encode batch");

    // Pair paths with embeddings
    let embeddings: Vec<(PathBuf, Vec<f32>)> = image_paths
        .iter()
        .take(sample_size)
        .cloned()
        .zip(embeddings_vec)
