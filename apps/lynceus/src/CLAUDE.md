...
- **The zero-importer export batch, remaining file**: un-export `tileUnderPoint` (services/telemetry.ts) is still open. Done: `lowestFreeY` (components/masonryPacking.ts), `useIndexingPhase`/`IndexingPhaseState` (hooks/useIndexingStatus.ts), `MasonryItemData` (components/Masonry.tsx) — landed 2026-08-03 (F4). The onboarding slice landed the same day (F3, `features/onboarding/CLAUDE.md`): the four `*_GRID` scene exports un-exported and `ARRANGE_TILE_FIXTURES` deleted outright as specified; `PRESS_DOWN_MS`/`PRESS_UP_MS`/`FADE_MS` (onboardingMotion.ts) were **deleted rather than un-exported** — un-exporting a zero-caller const fails `tsc --noEmit` under this project's `noUnusedLocals: true`, so the audit's own "defs only, no importers" framing meant deletion was the only build-clean move. tsc clean + 247/247 throughout. [code-health-audit 2026-08-02]
