...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-5 rounded-[14px] border border-border bg-card/88 py-5 text-card-foreground shadow-[var(--shadow-soft)]",
        className
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("text-[15px] font-[620] leading-tight tracking-[-0.02em]", className)}
      {...props}
    />
  )
}
