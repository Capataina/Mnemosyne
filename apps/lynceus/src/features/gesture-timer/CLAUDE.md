...
- **The reference-history strip (2026-08-03)**: bottom-left, under the "Reference N of M" label, small thumbnails for every reference shown this session, current one included. Pure viewing — clicking a past thumbnail never mutates the session.
  ...
  - **The strip's ring-clipping fix (2026-08-03, founder-reported the same day the strip shipped)**: same defect class as the similarity breadcrumb's 89b6ee2 fix — `overflow-x-auto` computes `overflow-y` to `auto` too (spec), and the strip had `pb-1` (bottom only), clipping the selected item's outward `ring-2` on its top edge and the unselected items' `ring-1` on theirs. Fixed with `p-1` (4px on all four sides) rather than switching to `ring-inset`: at a 44px (`size-11`) thumbnail an inset ring eats visible image area, while `gap-2` (8px) already clears two adjacent 2px rings meeting mid-strip, so padding-only was the cleaner fix. Ring radius (button's `rounded-md`, 6px) and the inner image's `rounded-[5px]` were checked and agree (inner slightly under outer, the correct direction for a box-shadow ring with no inset) — no corner mismatch, so left alone.
