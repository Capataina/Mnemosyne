...
## Planned work (2026-08-02 audit; all extraction targets live in `onboardingMotion.ts`, inside the feature)

- **Extract `pressAt(p)` over the 23 conforming press triples** (pattern extraction; gate-amended from the original ×25). A 29-case census against the live registry proved 25 press frames all carry exactly `(0.92, 0.75, 1)`, of which 23 are in-place arrive/press/release triples (AddFolder 3, Organise 6, Similarity 6, GesturePractice 6, Search 2); Arrange's two (ArrangeScene.tsx:239,245) are drag-presses — release at a different point with pressScale held — and are EXCLUDED by name. Settle: migration deep-equal against the registry, then the closedTracks suite. [code-health-audit 2026-08-02]
- **Extract `holdTrack(times, {hidden, shown, easeIn, easeOut})` over 21 of the 23 hidden-hold-hidden tracks** (pattern extraction; gate-amended from ×14+9). 26/28 reconstructions reproduce bit-identically (Search's glyph-0..5 via its own in-file factory — evidence the pattern factorises); two named exclusions stay hand-built unless the signature grows overrides: arrange/cTelegraph (plateau ease is `LIVE_EASE`, not `"linear"`; ArrangeScene.tsx:281) and add-folder/picker (exits through a third hidden frame, `-8px` translate; AddFolderScene.tsx:114-122). closedTracks binds identically post-extraction (endpoint closure holds by construction). [code-health-audit 2026-08-02]
- **Extract `reducedFrames(kinds)` over the six token-identical reduced-motion assembly blocks** (pattern extraction; gate-promoted). One `reducedKinds.map` per scene, identical modulo the three `[kind, caption]` pairs; helper home is `primitives/ReducedMotionFilmstrip.tsx` (needs JSX). Binding test: sceneRegistry.test.ts:19. [code-health-audit 2026-08-02]
- **Rename GesturePractice's `inspector` track to `setupView`** (naming; part of the five-item meaning batch, remainder in the parent `src/CLAUDE.md`). The scene's own filmstrip kind is `"gesture-setup"`; the key is file-local (declaration 104, TRACKS key 211, one visualMotion use 258). [code-health-audit 2026-08-02]
- Pointer: the tree-wide zero-importer batch in the parent `src/CLAUDE.md` also touches this feature — the four `*_GRID` scene exports and `PRESS_DOWN_MS`/`PRESS_UP_MS`/`FADE_MS` in `onboardingMotion.ts` un-export with that batch.
