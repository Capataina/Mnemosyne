...
│                       PinterestModal and the two edge slide-out panels
│                       (2026-08-03, geometry corrected same day: non-modal
│                       LibraryDrawer/SettingsDrawer anchored flush to the
│                       left/right screen edge, each driven by its own
│                       useBubbleTrigger() instance owned here — the route is
│                       the common ancestor of each trigger, in TopBar, and
│                       its panel. Passes `triggerProps` alongside
│                       `triggerRef`/`panelProps` to each panel now too, so
│                       each can wire its own edge hover strip
│                       (`EdgeHoverZone`, library-drawer/CLAUDE.md) to the
│                       same hover-intent handlers as the TopBar icon).
├── TopBar.tsx          Header: library + settings edge-panel triggers, wordmark, SearchBar,
...
│                       state itself, and is unchanged by the edge-slide-out geometry
│                       correction (the trigger buttons' own hover/pin behaviour never
│                       moved, only the panels' presentation). onSearchChange
│                       (leave-first invariant) and onAddFolder (duplicate-folder
│                       confirm flow) are prebuilt callbacks passed down — never
│                       rebuilt inside this component.
