...
- **Fix the generated report's dead brand string** (naming/clarity). `perf_report.rs:682` writes "Generated by the Pinterest Image Browser profiling system" into every `report.md` footer — two product renames stale. Refuter confirmed the string is asserted nowhere (no test, no parser) and ruled the change cosmetic-side of observable behaviour; profiling-artefact-only surface. [code-health-audit 2026-08-02]
- **Re-home the misattached doc block** (naming/clarity). The doc comment at `perf_report.rs:450-453` describes `section_diagnostics` but sits on `section_stall_analysis`; the real `section_diagnostics` (~line 621) is undocumented. Move the block; no doc-tests exist in the crate (`cargo test -p mnemosyne --doc` → 0), so the edit is inert. [code-health-audit 2026-08-02]
- **Reduce `perf::flush_to_file` to `pub(crate)`** (API surface). `perf.rs:393`; only callers are the in-crate flush thread (`perf.rs:380`) and `perf_report.rs:56`. The product crate re-exports the `perf` module wholesale (`src-tauri/src/lib.rs:28`) but nothing names this item through any path — refuter-proven by worktree dry-run, `cargo check --workspace --all-targets` clean, 141 lib + 24 integration tests green. Worktree caveat for the implementer: the `cha_*` proof tests and `models/` are untracked, so a HEAD worktree silently omits them — verify in the live tree or copy both in. [code-health-audit 2026-08-02]
