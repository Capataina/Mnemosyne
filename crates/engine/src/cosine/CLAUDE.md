...
- **RRF's tie order is HashMap-iteration order** — the fused sort is score-only, so images with equal RRF scores rank in an order that varies across launches (and would change under a hasher swap). Not hot (67 µs at production shape) and not a free fix (pinning tie order changes today's observable rankings) — but any "optimise the sort" idea must know a hasher change here is a behaviour change. [code-health-audit 2026-08-02]
- **The generation token cannot gate name-derived caches.** The token derives solely from the embeddings population (`images_query.rs:597-627`); the name catalogue reads `images.path`. A relink rename UPDATEs the path without touching embeddings (token unchanged → stale cached names on exactly the inputs a filename search targets), and never-encoded images — a documented load-bearing name-signal feature — have no embeddings row at all. Any token-gated cache over name data is structurally stale-serving; a name cache needs its own invalidation source. [code-health-audit 2026-08-02]

## Planned work

Ordered by severity. Proofs in `crates/engine/tests/`; benchmarks release-profile serial.

- **MED-HIGH — gate `populate_from_db_for_encoder`'s diagnostic on `is_profiling_enabled()`** (eliminated waste). The populate eagerly computes `embedding_stats` — three full store passes — as a `record_diagnostic` argument that profiling-off discards: 17.45 ms of a 64.55 ms populate at 10k×512 (27%), ~250 ms/encoder extrapolated at 100k×768, inside the fusion write-lock window. Distinct from the documented build-outside-lock deferral (this is pure waste, not lock restructuring). Proof: `cha_ret_populate_diag_waste.rs`. [code-health-audit 2026-08-02]
- **MED — `score_all` uses an indexed rayon collect** (data layout / parallel shape). The `filter_map` collect takes rayon's unindexed fallback; indexed `map` + `collect_into_vec` + retain is −43% (2.91 → 1.65 ms at 20k×512), equivalence asserted bitwise including row order; `dot_slice` untouched (the bit-identity boundary holds). Proof: `cha_ret_scan_collect_shape.rs`. [code-health-audit 2026-08-02]
- **LOW — `tokenize` single-pass lowercase** (eliminated waste). name_match's tokeniser lowercases in its own pass; folding it into the scan is −25%, exact-equivalence swept including Unicode edges. Proof: `cha_ret_name_match_bench.rs`. [code-health-audit 2026-08-02]
- **LOW — stream `save_store_to` instead of assembling the whole file in memory** (allocation churn). Byte-identical output proven across real-padding/zero-padding/single-row layouts and a 103 MB artefact; tmp+rename atomicity and failure cleanup preserved; wall-time neutral — the win is eliminating the `vec![0u8; total]` assembly buffer (103 MB at 50k×512, ~300 MB at 100k×768) down to a 64 B header + pad buffer. Proof: `cha_res_ret_stream_save.rs`. [code-health-audit 2026-08-02]

Two name-search accelerations were killed with reusable evidence: a gen-token-gated per-name tokenisation cache is structurally unsafe (see trap below), and banded Levenshtein LOST its own benchmark on all three query shapes (+5..22% at 20k names — filename strings are too short; band bookkeeping outweighs the ~40% of skipped cells; full equivalence was proven before benchmarking, numbers in git history at the audit commit). The name-scan cost itself is real — measured ~120-460 ms/query extrapolated at 100k, falsifying the module doc's "low tens of ms" — so the scan remains a legitimate future target, just not via these two routes. [code-health-audit 2026-08-02]
