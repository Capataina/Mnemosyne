...
- **HIGH — `mark_orphaned` gains `AND orphaned = 1`** (eliminated waste). `notes_orphans.rs:34`: the reset UPDATE rewrites every alive row on every pipeline pass because SQLite doesn't elide no-op UPDATEs — every steady-state 5s watcher rescan writes the whole root into the WAL under the writer mutex. Measured at 10k rows: 107,152 WAL bytes + 86.4 ms → 0 bytes + 10.9 ms. Gate-proven safe: rows-affected is discarded, no `changes()` reader exists anywhere, relink observes bit-identical state including the mid-pass window. Proof: `cha_b_mark_orphaned_rewrite.rs` (state + return-count equivalence, four scenarios). [code-health-audit 2026-08-02]
- **MED — Ready count via `COUNT(*)`, not `get_all_images().len()`** (eliminated waste). Caller at `apps/lynceus/src-tauri/src/indexing.rs:1008`: a full tag-join unroll + HashMap + materialise + sort for one integer, every pipeline pass. `get_all_images` has no WHERE, so bare `COUNT(*)` on the same (writer) connection is byte-identical including orphaned/disabled rows; 64 ms → 2.4 µs release-serial at 20k×3 tags (two seats converged independently). Proofs: `cha_l3_db_waste.rs`, `cha_b_ready_count.rs`. [code-health-audit 2026-08-02]
- **MED — the thumbnail needs-set query drops its tag join** (eliminated waste). `images_query.rs:244` (`get_images_without_thumbnails`): callers read only id/path/name, the join+aggregate is discarded. 85 ms → 2.6 ms (33×) at 8k×3 tags. Implementation notes from the gate: keep it on the writer connection (today's routing) and add `ORDER BY id`; the apparent second caller at `thumbnail/generator.rs:434` is inside a zero-caller dead method, so the migration surface is one live call site. Proofs: `cha_b_thumbnail_needs_shape.rs`, `cha_b_needs_count.rs`. [code-health-audit 2026-08-02]
- **MED — split `images_query.rs` (1573 lines) at the T3-1 seam into `db/feed_manifest.rs`** (modularisation). Address-only move of the feed-manifest impl block (lines 769-998), `FeedManifestRow`, `basename_of`, and the four T3-1 tests (cut at the `#[test]` attribute above line 1410 — not 1402, refuter-corrected). Complete batch, refuter-verified: `pub mod feed_manifest` in `db/mod.rs`; the one cross-crate edge `apps/lynceus/src-tauri/src/commands/images.rs:7` SPLITS its import (`FeedManifestRow` moves, `PipelineStats` stays); `aggregate_image_rows` widens fn → `pub(super)` (dry-run-proven; its private `AggregatedRow` alias is transparent to type privacy); moved tests keep `use super::super::test_helpers::fresh_db` verbatim; no shared test helper crosses the split (`setup_tagged_images` stays with the staying tests). [code-health-audit 2026-08-02]
- **LOW — `get_preview_eligibility` in one pass** (eliminated waste). Three per-bucket `images` scans → one `SUM(CASE)` scan; 1.9-2.0×, same connection, and safer — one snapshot closes today's cross-bucket inconsistency window. Proof: `cha_l3_db_waste.rs`. [code-health-audit 2026-08-02]
- **LOW — `aggregate_image_rows` allocates lazily** (allocation churn). Eager `or_insert` clones each image's Strings once per tag row; lazy-on-vacant is 1.39×, output identical (proof covers both `get_all_images` call sites). Proof: `cha_l3_alloc_and_hash.rs`. [code-health-audit 2026-08-02]
- **LOW — single-copy BLOB extraction via `get_ref` in `embeddings.rs`** (allocation churn). Halves copy volume, −5% (7.19 → 6.85 ms at 10k×512); Ok-path identical incl. malformed-skip, TEXT-error Debug-identical, NULL pinned unreachable (NOT NULL, code 1299). Two riders from the resolver: copy via the aligned destination (a borrowed SQLite blob pointer would make `cast_slice` panic on alignment), and the change deliberately fixes a latent panic the proof pinned — bytemuck's alignment check panics on a dangling empty-Vec pointer for empty BLOBs, an encoder-unreachable input today. Proof: `cha_res_ret_blob_get_ref.rs`. [code-health-audit 2026-08-02]
- **LOW — correct the stale double-write doc comments** (doc rot). `embeddings.rs:149-154` and `db/mod.rs:257-261` still claim the pipeline double-writes the legacy `images.embedding` column — false since R8; both live pipeline call sites pass `legacy_clip_too = false`, and the one unconditional legacy writer (`update_image_embedding`) has zero non-test callers (refuter-traced). No doc-tests in the crate. [code-health-audit 2026-08-02]
