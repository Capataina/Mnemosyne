...
// pub(super): `feed_manifest.rs`'s `get_image_details_by_ids` reuses
// this helper too (dry-run-proven; the private `AggregatedRow` alias
// stays transparent to type privacy) [code-health-audit 2026-08-02].
pub(super) fn aggregate_image_rows(
...
impl ImageDatabase {
    ...
    pub fn get_images_without_thumbnails(&self) -> rusqlite::Result<Vec<ThumbnailNeedRow>> {
        ...
        for row in rows {
            let (id, path) = row?;
            // basename_of lives in the sibling feed_manifest module now
            // (T3-1 split); pub(super) the other way so this needs-set
            // read still derives the same display name for its
            // `feed-delta` rows [code-health-audit 2026-08-02].
            let name = super::feed_manifest::basename_of(&path);
