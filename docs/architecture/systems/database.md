...
| `save_to_disk` / `paths::cosine_cache_path` orphaned by the T3-2/#8 rework | The primary `CosineIndexState` was removed entirely in `1514a90` | Public, tested, but caller-less — flagged in `docs/engineering/decisions/performance-decisions.md`, left in place for the next pass through that file rather than deleted here. |
