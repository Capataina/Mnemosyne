...
*Maturity: working · Stability: stable core, gained a batched `encode_batch` override this round (T2-3, `ebe4006`) — verified equivalent to serial encode, not a behaviour change*
...
Its embeddings feed `systems/multi-encoder-fusion.md`'s image-image RRF fusion across all three encoders (DINOv2 is implicitly excluded from text-image fusion since it's image-only); produced/consumed by the `Phase::Encode` sequence in `systems/indexing.md`.
...
- **Public API:** `Dinov2ImageEncoder::new(model_path)` plus the trait impl, with an **overridden `encode_batch`** (T2-3, commit `ebe4006`) — see Current Implemented Reality below. Constants: `DINOV2_IMAGE_MODEL_URL`, `DINOV2_IMAGE_MODEL_FILENAME`, `DINOV2_ENCODER_ID`. Implements the `ImageEncoder` trait so the indexing pipeline dispatches via `Box<dyn ImageEncoder>`.
...
### Batch inference override (T2-3, commit `ebe4006`) — one `[N,3,224,224]` session call per chunk

Xenova's DINOv2-Base export also declares **dynamic** batch dims, so `Dinov2ImageEncoder::encode_batch` overrides the `ImageEncoder` trait's per-image default instead of falling through to it — the same pattern as SigLIP-2 (`siglip2-encoder.md`), applied to the CLS-token output shape instead of `pooler_output`:
...
encode_batch(paths):
    for chunk in paths.chunks(32):                 // INFER_BATCH — bounds peak tensor memory
        ...
        onnx_input = Tensor::from_array(([n, 3, 224, 224], batch_data))
        outputs = session.run(pixel_values: onnx_input)   // ONE call for the whole chunk
        last_hidden_state  shape [n, 257, 768]  (257 = 1 CLS + 256 patch tokens per image)
        for i in 0..n:
            base = i * 257 * 768                   // each image's seq×hidden slab
            all_embeddings.push(normalize(&data[base .. base+768]))   // CLS = first row of the slab
```

Preprocessing (resize + crop + normalise) is unchanged per-image; only the ONNX call is batched, replacing N single-image `[1,3,224,224]` calls with one `[N,3,224,224]` call per 32-chunk — roughly **1.2–2× on CPU-only ORT** (CoreML disabled here too), not a GPU-style collapse.

**Failure semantics are preserved exactly.** Same reasoning as SigLIP-2's override: the trait default's `collect()` already short-circuits on the first failing image and fails the whole chunk, and the batched override reproduces that via `?` on preprocessing rather than adding per-image isolation. The `pooler_output` defensive-fallback branch (see above) is mirrored in the batched path too, in case a future export adds a pooler.

**Equivalence, verified against real models** (`tests/batched_encode_equivalence_diagnostic.rs`, `#[ignore]`, needs `dinov2_base_image.onnx` on disk): batched vs. serial `encode` per image at **cosine ≥ 0.9999996** (per-image float-accumulation-order differences between a batch-of-N matmul and N separate batch-of-1 matmuls, not exact 1.0). The committed test's assertion gate is a looser `cosine > 0.999`; the ≥0.9999996 figure is the measured result from the `ebe4006` commit body.
...
indexing.rs::run_trait_encoder("dinov2_base", make_encoder=Dinov2ImageEncoder::new):
    ...
    if dinov2_path.exists():
        ...
        for chunk in needs.chunks(32):
            embeddings = encoder.encode_batch(chunk_paths)   // ONE [N,3,224,224] session call — see above
...
Two chunk boundaries stack here, same as SigLIP-2: `indexing.rs` hands ≤32 paths per call, and `encode_batch` re-chunks internally by its own `INFER_BATCH = 32` (redundant today, a defensive cap for future callers). See `systems/indexing.md` for full Phase::Encode sequencing across all three encoder families.
...
| Batched `encode_batch` preserves the trait default's whole-chunk failure semantics (T2-3) | A single corrupted image inside a 32-image chunk | The `?`-short-circuit on preprocessing fails the ENTIRE chunk of up to 32 images, same as before batching landed — a bad image's chunk-mates get recorded as failed too. Preserved behaviour, not a regression, but worth knowing if failure counts look surprising. |
...
- **Batching the ONNX call was contingent on the export's dynamic batch dim, same constraint as SigLIP-2.** Xenova's DINOv2-Base export declares a symbolic batch dimension, which is what makes the T2-3 override safe; CLIP's separate `visual/model.onnx` export has a FIXED batch dim of 1 and would error if the same approach were tried there (see `clip-image-encoder.md` Durable Notes). The override preserves the trait default's whole-chunk failure semantics deliberately — the wave-1 commit corrected an earlier, incorrect assumption that the trait default already isolated a bad image's failure from its chunk-mates.
