...
### Masonry layout monitor (`startLayoutMonitor`)

> **Note (2026-07-17):** this section was added at wrap-up to record a capability that landed after the last full upkeep pass; the surrounding telemetry-v2 section above was not re-audited against it. The surrounding telemetry-v2 material predates this addition; verify it against source and update this named document directly before relying on it.

A telemetry surface built specifically to make masonry reflow bugs *observable* rather than guessed (it was the instrument behind the teleport and reorder-drift fixes, `91564f0`/`e3adc2e`). The monitor is event-armed rather than free-running: capture-phase pointerdown/held-pointermove handlers arm it for gestures, while a masonry-scoped `MutationObserver` catches non-pointer reflows such as feed-delta merges. It samples every second animation frame while a drag is active, a reflow is settling, or motion was seen in the previous 1.5 seconds, then stops completely during idle browsing. Persistent current/previous maps remove per-sample map allocation, and each sample finishes all `getBoundingClientRect` reads before the separate `getComputedStyle` phase.

When motion starts the monitor captures the gesture context (dragging / grabbed id / cursor), and after about six still browser frames (three half-rate samples) it emits **one** `reflow` event summarising the whole reflow rather than a per-frame stream. Each event carries: `trigger` (drag vs other + cursor + grabbed id), a per-tile `moved` list (`{id, total, dx, dy, maxFrameJump, verdict, dir, transProp, transDur}`), `mounted`/`unmounted` id lists, `teleportCount`, and a `settled` geometry snapshot. The per-tile **verdict** comes from the pure, unit-tested `classifyMove`: the historical >60%-per-frame boundary scales with the sample span, so at the two-frame cadence only a complete between-sample jump is a `TELEPORT`; `transProp`/`transDur` (captured via `getComputedStyle` at the biggest jump) record whether the CSS transition was live. `captureGridGeometry()` reports per-tile pack-vs-render divergence (`mismatched`), pairwise `overlaps` (via `tilesOverlap`), and vertical column `gaps` (>40px, `{x, gap, above: upperTileId, below: lowerTileId}` — note `above`/`below` are tile **ids**, not y-coordinates), and `tileUnderPoint(x,y)` resolves what sits under the cursor (a tile id or `"EMPTY"`). This replaced an earlier set of piecemeal per-drop/per-drag samplers (`2086e13` unified them into the single monitor). Same profiling gate and `record_user_action` sink as everything above — no backend change.
