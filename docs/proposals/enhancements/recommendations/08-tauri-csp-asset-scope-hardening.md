...
A short `docs/engineering/decisions/security-posture.md` documents the threat model: WebView is sandboxed from system, asset protocol now bounded to declared roots, CSP prevents injected-script RCE.
...
4. Document the threat model in `docs/engineering/decisions/security-posture.md`. ~½ day.
