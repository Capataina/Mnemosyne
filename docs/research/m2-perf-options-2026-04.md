...
The remaining options (1, 4, 5, 6, 7, 9, 10) are still open — verified absent from the tree: no `fp16`/`float16`/`half::f16` symbols, no `int8` model files (`models/image/` holds FP32 `.onnx` weights only), no `mozjpeg`/`turbojpeg`/`zune` dependency, no `candle`/`mlx-rs` dependency. The decode-once fan-out (option 7) is tracked as a deferred item with its reopen trigger in `docs/engineering/decisions/performance-decisions.md`.

**Update (2026-07-17, 100k performance round).** A form of batching not enumerated as one of the ten options above shipped: SigLIP-2 and DINOv2 now override the encoder trait's one-by-one inference default with a single `[N,3,H,W]` ONNX session call per chunk (`ebe4006`), rather than looping `session.run()` once per image — distinct from A7's "batch size 16-32 sweet spot" note above, which was about the *tensor's* batch dimension, not the *call count*. Equivalence proven against real weights (per-image cosine ≥ 0.9999996 for both encoders). CLIP stays per-image: its ONNX export has a fixed batch dim of 1, and re-exporting it touches the live weights-provenance question in `docs/engineering/decisions/clip-preprocessing-decisions.md`, so it's tracked there rather than here.
...
| 7. Decode-once buffer share between thumbnail and encoder steps | M — pipeline restructure | 1.3–1.5x on the JPEG-decode wall-clock (which is ~half of thumbnail step today) | None | High — **open (tracked in `docs/engineering/decisions/performance-decisions.md`)** |
